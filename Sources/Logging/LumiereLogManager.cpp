...
void LumiereLog(const std::string& message, const std::string& fileName, const std::string& functionName, int lineNumber, ELogLevel logLevel)
{
    LUMIERE_EXPECT(!message.empty() && !fileName.empty() && !functionName.empty());
...
{
    LUMIERE_EXPECT(!fileName.empty() && !functionName.empty() && lineNumber >= 0);
}
